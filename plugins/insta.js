...
Julie.addCommand({ pattern: 'insta ?(.*)', fromMe: false, desc: "Downloads from instagaram", dontAddCommandList: true }, async (message, match) => { 
    ...
    if (type === 'image') return await message.sendMessage(data, MessageType.image, { caption: "𝙼𝚊𝚍𝚎 𝚋𝚢 𝚕𝚞𝚝𝚝𝚊𝚙𝚒😈", quoted: message.data })
    if (type === 'video') return await message.sendMessage(data, MessageType.video, { caption: "𝙼𝚊𝚍𝚎 𝚋𝚢 𝚕𝚞𝚝𝚝𝚊𝚙𝚒😈", quoted: message.data })
});
